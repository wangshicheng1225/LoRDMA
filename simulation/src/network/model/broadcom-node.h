...
namespace ns3 {
	...
	public:
		...
		void GetPauseClasses(uint32_t port, uint32_t qIndex, bool pClasses[], uint32_t node_id);
		...
		uint32_t GetUsedIngressPGBytes(uint32_t port, uint32_t qIndex);
		uint32_t GetUsedIngressPortBytes(uint32_t port);
		uint32_t GetUsedIngressPGHeadroomBytes(uint32_t port, uint32_t qIndex);
