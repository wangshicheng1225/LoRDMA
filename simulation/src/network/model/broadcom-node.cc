...
namespace ns3 {
	...
	BroadcomNode::BroadcomNode()
	{
		...
		//ingress params
		m_buffer_cell_limit_sp = 4 * 1024 * 1024; //ingress sp buffer threshold   =guaranteed + shared buffer
		...
		m_port_max_shared_cell = 2400 * 1030; //max buffer for an ingress port
		...
		BroadcomNode::GetPauseClasses(uint32_t port, uint32_t qIndex, bool pClasses[])
	{
		if (m_dynamicth)
		{
			for (uint32_t i = 0; i < qCnt; i++)
			{
				...
				{
					//std::cout << "m_pg_min_cell=" << m_pg_min_cell << ", m_port_min_cell=" << m_port_min_cell << ", m_pg_shared_alpha_cell=" << m_pg_shared_alpha_cell << "\n";
					pClasses[i] = true;
				}
			}
		}
	...
	void BroadcomNode::GetPauseClasses(uint32_t port, uint32_t qIndex, bool pClasses[], uint32_t node_id)
	{
		if (m_dynamicth)
		{
			for (uint32_t i = 0; i < qCnt; i++)
			{
				...
				if (i == 1 && !m_enable_pfc_on_dctcp)			//dctcp
					continue;
				/*if (node_id == 0)
				std::cout << "m_usedIngressPGBytes[port][i]=" << m_usedIngressPGBytes[port][i] << ", m_pg_min_cell=" << m_pg_min_cell << ", m_port_min_cell=" << m_port_min_cell << ", m_pg_shared_alpha_cell=" << m_pg_shared_alpha_cell << ", m_buffer_cell_limit_sp=" << m_buffer_cell_limit_sp << ", m_usedIngressSPBytes[GetIngressSP(port, qIndex)]=" << m_usedIngressSPBytes[GetIngressSP(port, qIndex)] << "\n";*/
				if ((double)m_usedIngressPGBytes[port][i] - m_pg_min_cell - m_port_min_cell > m_pg_shared_alpha_cell*((double)m_buffer_cell_limit_sp - m_usedIngressSPBytes[GetIngressSP(port, qIndex)]))
				{
					pClasses[i] = true;
				}
		...
		BroadcomNode::ShouldSendCN(uint32_t indev, uint32_t ifindex, uint32_t qIndex)
	{
		...
		else
		{
			...
			{
				//std::cout << "m_pg_qcn_threshold=" << m_pg_qcn_threshold << ", m_pg_qcn_threshold_max=" << m_pg_qcn_threshold_max << "\n";
	...
	uint32_t BroadcomNode::GetUsedIngressPGBytes(uint32_t port, uint32_t qIndex)
	{
		return m_usedIngressPGBytes[port][qIndex];
	}

	uint32_t BroadcomNode::GetUsedIngressPortBytes(uint32_t port)
	{
		return m_usedIngressPortBytes[port];
	}

	uint32_t BroadcomNode::GetUsedIngressPGHeadroomBytes(uint32_t port, uint32_t qIndex)
	{
		return m_usedIngressPGHeadroomBytes[port][qIndex];
	}
