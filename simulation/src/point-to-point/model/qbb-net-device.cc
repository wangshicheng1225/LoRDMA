...
namespace ns3 {
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddAttribute("L2WaitForAckTimer",
				...
				MakeDoubleChecker<double>())
			.AddAttribute("CCEnabled",
				"RP runs CC when receiving a CNP",
				BooleanValue(true),
				MakeBooleanAccessor(&QbbNetDevice::m_ccEnabled),
				MakeBooleanChecker())
			.AddAttribute("TotalNicNum",
				"Number of nics in topology",
				UintegerValue(0),
				MakeUintegerAccessor(&QbbNetDevice::m_total_nic_num),
				MakeUintegerChecker<uint32_t>())
			;
	...
	QbbNetDevice::QbbNetDevice()
	{
		...
		m_datarate_path = "mix/datarate.tr";
		m_qlen_path = "mix/qlen.tr";
		m_rtt_path = "mix/rtt.tr";
		m_ack_path = "mix/ack.tr";
		m_probe_path = "mix/probe_path.tr";
		m_ecmp_path = "mix/ecmp.tr";
		m_congestion_path = "mix/congestion.tr";
		m_portrate_path = "mix/portrate.tr";

		//m_datarate_file.open(datarate_path.c_str(), std::ios::app);
		m_last_datarate_time = 0;
		m_last_portrate_time = 0;
		m_EPS = 1e-4;                
		m_pfc_pause_status = false;
		m_last_qlen_time = 0;
		m_qlen_EPS = 1e-6;
		//monitorSendingRate();
	}
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		if (p != 0)
		{
			...
			if (m_node->GetNodeType() == 0) //I am a NIC, do QCN
			{
				...
				if (fabs(m_last_datarate_time - Simulator::Now().GetSeconds()) < m_EPS || m_pfc_pause_status || m_rate[fIndex].GetBitRate() == 0)
				{
					//std::cout << "equal\n";
				}
				else
				{
					//Print datarate
					m_last_datarate_time = Simulator::Now().GetSeconds();
					m_datarate_file.open(m_datarate_path.c_str(), std::ios::app);
					m_datarate_file << Simulator::Now().GetSeconds() << " " << m_node->GetId() << " " << m_rate[fIndex] << "\n";
					m_datarate_file.close();
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		Ipv4Address proberAddr("10.3.165.2");
		Ipv4Address receiverAddr("10.1.224.2");
		...
		{ //This is not QCN feedback, not NACK, or I am a switch so I don't care
			...
			else // If this is a Pause, stop the corresponding queue
			{
				...
				if (1)
				{
					if (!m_qbbEnabled) return;    //m_qbbEnabled indicates whether the NIC responds to the switch's PFC pause frames
					PauseHeader pauseh;
					...
					if (pauseh.GetTime() > 0)
					{
						////Print PFC pause frames
						////std::cout << "Pause Packet Receive, time=" << Simulator::Now().GetSeconds() << ", node=" << m_node->GetId() << "\n";
						//m_congestion_file.open(m_congestion_path.c_str(), std::ios::app);
						//m_congestion_file << "Pause Packet Receive, time=" << Simulator::Now().GetSeconds() << ", node=" << m_node->GetId() << "\n";
						//m_congestion_file.close();

						if (fabs(m_last_datarate_time - Simulator::Now().GetSeconds()) < m_EPS || m_node->GetId() == m_total_nic_num)
						{
							//std::cout << "equal\n";
						}
						else
						{
							//Print dararate
							//std::cout << "m_last_datarate_time=" << m_last_datarate_time << ", Simulator::Now().GetSeconds()=" << Simulator::Now().GetSeconds() << "\n";
							m_last_datarate_time = Simulator::Now().GetSeconds();
							m_datarate_file.open(m_datarate_path.c_str(), std::ios::app);
							m_datarate_file << Simulator::Now().GetSeconds() << " " << m_node->GetId() << " 0bps" << "\n";
							m_datarate_file.close();
						}

						m_pfc_pause_status = true;
		...
		else if (ipv4h.GetProtocol() == 0xFF) //
		{
			...
			if (ecnbits == 0x03 && m_ccEnabled)
			{

				m_congestion_file.open(m_congestion_path.c_str(), std::ios::app);
				m_congestion_file << "rpr_cnm_received, time=" << Simulator::Now().GetSeconds() << ", id=" << m_node->GetId() << "\n";
				m_congestion_file.close();
		...
		QbbNetDevice::CheckQueueFull(uint32_t inDev, uint32_t qIndex)
	{
		...
		for (uint32_t j = 0; j < qCnt; j++)
		{
			...
			{
				//std::cout << "time=" << Simulator::Now().GetSeconds() << ", node=" << m_node->GetId() << ", m_usedIngressPGBytes=" << m_node->m_broadcom->GetUsedIngressPGBytes(inDev, qIndex) << ", m_usedIngressPortBytes=" << m_node->m_broadcom->GetUsedIngressPortBytes(inDev) << ", m_usedIngressPGHeadroomBytes=" << m_node->m_broadcom->GetUsedIngressPGHeadroomBytes(inDev, qIndex) << "\n";
				//std::cout << Simulator::Now().GetSeconds() << ", " << m_queue->GetNPackets() << "\n";
	...
	void
		QbbNetDevice::CheckandSendQCN(Ipv4Address source, uint32_t qIndex, uint32_t port) 
	{
		if (m_node->GetNodeType() > 0)
			return;
		if (!m_qcnEnabled)
	...
	{
		//std::cout << "node=" << m_node->GetId() << ", m_rpByteStage[fIndex][hop]=" << m_rpByteStage[fIndex][hop] << ", m_rpTimeStage[fIndex][hop]=" << m_rpTimeStage[fIndex][hop] << "\n";
		if (m_rpByteStage[fIndex][hop] < m_rpgThreshold || m_rpTimeStage[fIndex][hop] < m_rpgThreshold)
		{
			//std::cout << "rpr_active_increase, time=" << Simulator::Now().GetSeconds() << "\n";
			rpr_active_increase(fIndex, hop);
		}
		else
		{
			//std::cout << "rpr_hyper_increase, time=" << Simulator::Now().GetSeconds() << "\n";
			rpr_hyper_increase(fIndex, hop);
		}
		return;
	}
		...
		QbbNetDevice::rpr_cnm_received(uint32_t findex, uint32_t hop, double fraction)  
	{
		...
		//std::cout << "m_alpha=" << m_alpha[findex][hop] << "\n";
