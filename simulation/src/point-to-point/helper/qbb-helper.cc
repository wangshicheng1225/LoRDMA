...
namespace ns3 {
	...
	NetDeviceContainer
		QbbHelper::Install(Ptr<Node> a, Ptr<Node> b, bool dst_cc_enabled, uint32_t node_num)
	{
		NetDeviceContainer container;
		Ptr<QbbNetDevice> devA = m_deviceFactory.Create<QbbNetDevice>();
		devA->SetAddress(Mac48Address::Allocate());
		devA->SetAttribute("TotalNicNum", UintegerValue(node_num));
		//devA->SetAttribute("QcnEnabled", BooleanValue(src_is_qcn_enabled));
		a->AddDevice(devA);
		Ptr<QbbNetDevice> devB = m_deviceFactory.Create<QbbNetDevice>();
		devB->SetAddress(Mac48Address::Allocate());
		//devB->SetAttribute("QcnEnabled", BooleanValue(dst_cc_enabled));
		devB->SetAttribute("TotalNicNum", UintegerValue(node_num));
		devB->SetAttribute("CCEnabled", BooleanValue(dst_cc_enabled));
		b->AddDevice(devB);
